...
<html lang="ko">
<head>
	<title>홈 > 커뮤니티 > 커뮤니티 관리 > 리스트</title>
	<script>
		$(function(){
			...
			*/
			...
			$("#searchBtn").bind("click", function(){
				$("#searchForm").attr('action', '<c:url value="/CM/UF/UF03" />').submit();
				return false;
			});
			...
			$(document).on('click', '.btnRegBoard', function(){
				...
				//alert(communitySeq );
				location.href = '<c:url value="/CM/UF/UF0303"/>?communitySeq=' + communitySeq + '&gameNm=' + gameNm;
				
			});
			//게시글 목록 보기
			$(document).on('click', '.btnBoardList', function(){
				var communitySeq = $(this).data('seq');
				var gameNm = $(this).data('gamenm');
				
				//alert(communitySeq );
				location.href = '<c:url value="/CM/UF/UF02"/>?communitySeq=' + communitySeq + '&gameNm=' + gameNm;
			});
...
<body>
	...
	<p class="location">
		...
		<strong>커뮤니티 리스트</strong>
	</p>
	<div class="inner-cont">
		<div class="title-area">
			<!-- Title -->
			<h2>커뮤니티 리스트<c:if test="${params.gameNm != null and params.gameNm != ''}"> - <font color="blue">${params.gameNm}</font></c:if> </h2>
			<p>커뮤니티를 관리합니다.<span class=""></span></p>
		</div>
		<!-- search -->
		<div class="search">
							...
							<div class="search-select">
								<div class="select-row1">
									<span class="search-group">
										<select title="사용여부" id="useYn" name="useYn">
											<option value="">전체</option>
											<option value="Y" <c:if test="${params.useYn == 'Y'}">selected</c:if> >사용</option>
											<option value="N" <c:if test="${params.useYn == 'N'}">selected</c:if> >미사용</option>
										</select>
									</span>
								</div>
						...
						<c:when test="${fn:length(list) > 0 }">
							<c:forEach var="item" items="${list}" varStatus="status">
								<tr>
									...
									<td>${item.modRegId }</td>
									<td>
										<button class="btn-black3 btnBoardList" data-seq="${item.seqNo}" data-gamenm="${item.gameNm}"><span>게시글목록</span></button>
										<p>
										<button class="btn-black3 btnRegBoard" data-seq="${item.seqNo}" data-gamenm="${item.gameNm}"><span>게시글쓰기</span></button>
									</td>
...
<div id="boardRegPop" class="modal-pop">
	<div class="modal-cotainer">
		<div class="modal-content">
			<!-- content -->
			<h2 class="tit">게시글쓰기</h2>
