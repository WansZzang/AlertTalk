	...
	<script>
		$(function(){
			$('.btnAdd').click(function(){
				location.href = '<c:url value="/AM/PO/PO03"/>';
			});
			...
			$(document).on('click','table > tbody > tr > td:nth-child(3)', function(){
				...
				location.href = '<c:url value="/AM/PO/PO04"/>?seqNo=' + _seqNo;
			});
			...
			$("#searchBtn").bind("click", function(){
				...
				if(listType == 1){
					$("#searchForm").attr('action', '<c:url value="/AM/PO/PO01" />').submit();	
				}else{
					$("#searchForm").attr('action', '<c:url value="/AM/PO/PO11" />').submit();
				}
			...
			$("#excelBtn").bind("click", function(){
				var listType = $('input[name=listType]').val();
				if(listType == 1){
					$("#searchForm").attr('action', '<c:url value="/AM/AM06" />?areaCd=PO&listType=1').submit();
				}else{
					$("#searchForm").attr('action', '<c:url value="/AM/AM06" />?areaCd=PO&listType=2').submit();
				}
				return false;
			});
		});
