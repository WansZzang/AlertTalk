	...
	<script>
		$(function(){
			$('.btnAdd').click(function(){
				location.href = '<c:url value="/AM/TO/TO03"/>';
			});
			...
			$(document).on('click','table > tbody > tr > td:nth-child(3)', function(){
				...
				location.href = '<c:url value="/AM/TO/TO04"/>?seqNo=' + _seqNo;
			});
			...
			$("#searchBtn").bind("click", function(){
				...
				if(listType == 1){
					$("#searchForm").attr('action', '<c:url value="/AM/TO/TO01" />').submit();
				}else{
					$("#searchForm").attr('action', '<c:url value="/AM/TO/TO11" />').submit();
				}
				return false;
			});
			...
			$("#excelBtn").bind("click", function(){
				var listType = $('input[name=listType]').val();
				if(listType == 1){
					$("#searchForm").attr('action', '<c:url value="/AM/AM06" />?areaCd=TO&listType=1').submit();
				}else{
					$("#searchForm").attr('action', '<c:url value="/AM/AM06" />?areaCd=TO&listType=2').submit();
				}
				return false;
			});
		});
...
<body>
	...
	<div class="inner-cont">
		<div class="title-area">
			<!-- Title -->
			<h2>Top</h2>
			<p>Top을 관리합니다.<span class=""></span></p>
		</div>
