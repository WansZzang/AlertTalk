...
<html lang="ko">
<head>
	<title>홈 > 커뮤니티 > 커뮤니티 관리 > 댓글 관리(게시글)</title>
	<script>
		$(function(){
			/* 
			...
			*/
			...
			$("#searchBtn").bind("click", function(){
				$("#searchForm").attr('action', '<c:url value="/CM/UF/UF01" />').submit();
				return false;
			});
			...
			$(document).on('click', '.btnRegReply', function(){
				...
				//alert(boardSeq + ', ' + communitySeq );
				
				$('#popupIfrm').ready(function () {
					showLayer($('.btnRegReply'), 'commentRegPop');
				});
				$('#popupIfrm').attr('src', '<c:url value="/popup/CM/UF/UF05"/>?boardSeq=' + boardSeq + '&communitySeq=' + communitySeq + '&parentId=' + 1); 
...
<body>
	...
	<p class="location">
		...
		<strong>댓글 관리(게시글)</strong>
	</p>
	<div class="inner-cont">
		<div class="title-area">
			<!-- Title -->
			<h2>댓글 관리(게시글)<c:if test="${params.gameNm != null and params.gameNm != ''}"> - <font color="blue">${params.gameNm}</font></c:if> </h2>
								...
								<div class="select-row3">
									<span class="search-group">
										<select title="검색대상" name="searchType2" id="searchType2">
											...
											<option value="3" <c:if test="${param.searchType2 == '3'}">selected</c:if> >댓글</option>
											<option value="4" <c:if test="${param.searchType2 == '4'}">selected</c:if> >닉네임</option>
										</select>
									</span>
						...
						<c:when test="${fn:length(list) > 0 }">
							<c:forEach var="item" items="${list}" varStatus="status">
								<tr>
									...
									<td>${item.gameNm }</td>
									<td>
										${fn:replace(item.boardComment, '&lt;br&gt;', '<br>') }
									</td>
									<td>${item.comment }</td>
									...
									<td>
										<button class="btn-black2 btnRegReply" data-boardseq="${item.boardSeq}" data-communityseq="${item.communitySeq}"><span>댓글달기</span></button>
									</td>									
								</tr>
							</c:forEach>
						</c:when>
