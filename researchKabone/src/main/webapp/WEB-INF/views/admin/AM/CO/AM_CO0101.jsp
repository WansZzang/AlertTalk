	...
	<script>
		$(function(){
			$('.btnAdd').click(function(){
				location.href = '<c:url value="/AM/CO/CO03"/>';
			});
			...
			$(document).on('click','table > tbody > tr > td:nth-child(3)', function(){
				...
				location.href = '<c:url value="/AM/CO/CO04"/>?seqNo=' + _seqNo;
			});
			...
			$("#searchBtn").bind("click", function(){
				$("#searchForm").attr('action', '<c:url value="/AM/CO/CO01" />').submit();
				return false;
			});
			
			//excel
			$("#excelBtn").bind("click", function(){
				$("#searchForm").attr('action', '<c:url value="/AM/AM06" />?areaCd=CO').submit();			
				return false;
			});
		});
...
<body>
	...
	<div class="inner-cont">
		<div class="title-area">
			<!-- Title -->
			<h2>Content</h2>
			<p>Content를 관리합니다.<span class=""></span></p>
		</div>
		<!-- search -->
		<div class="search">
